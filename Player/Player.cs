...
public partial class Player : CharacterBody3D {
	...
	public void RequestDropWeapon() {
		if (!_isHoldingWeapon) return;

		if (Multiplayer.HasMultiplayerPeer() && IsMultiplayerAuthority()) {
			Rpc(nameof(RpcDropWeapon));
		} else {
			DropWeapon();
		}
	}
	...
	public override void _Input(InputEvent @event) {
		...
		// Ataque Cuerpo a Cuerpo Sincronizado (Clic Izquierdo / Acción "shoot")
		...
		if (isAttackPressed && _isHoldingWeapon) {
			if (Multiplayer.HasMultiplayerPeer()) {
				Rpc(nameof(RpcTriggerAttack));
			} else {
				RpcTriggerAttack();
			}
		}

		// Soltar Arma Sincronizado (Tecla G / Acción "drop")
