...
@export var balas_reserva: int = 20
@export var tiempo_recarga: float = 0.7
...
func _physics_process(_delta: float) -> void:
	# 🔥 CORRECCIÓN CRÍTICA: Si el arma está en el suelo, ignora por completo los botones del jugador
	if _portador == null: 
		return
...
func _intentar_disparar() -> void:
	...
	if _animador != null and _animador.has_animation("recoil"):
		_animador.stop()
...
func _iniciar_recarga() -> void:
	...
	if _animador != null and _animador.has_animation("reload"):
		_animador.stop()
...
func _on_pickup_area_body_entered(body: Node3D) -> void:
	...
	if mano_jugador != null:
		...
		scale = Vector3.ONE # 🔥 FIX: Asegura que la pistola se vea y no se encoja al equiparse
...
func _on_animador_animation_finished(anim_name: StringName) -> void:
	if anim_name == "recoil" or anim_name == "Equipar":
		_puede_disparar = true
